...
App 使用了 **Material Design 3**（安卓最新设计语言），支持：
- ☀️ 浅色 / 🌙 深色模式自动切换
- 🎨 跟随手机壁纸的动态取色
- 📱 适配不同屏幕尺寸
...
### 1️⃣ 看课表（打开就是）
App 打开后自动进入课表 → 本周课程一目了然 → 左右切换查看其他周 → 下拉刷新获取最新课表。

### 2️⃣ 查成绩
左上角菜单 → 「成绩」→ 看到所有学期的成绩 → 顶部显示当前 GPA → 点击某门课可看详细得分构成（平时分、期末分各占多少）。

### 3️⃣ 找空教室
菜单 → 「空教室」→ 选校区和教学楼 → 选周次、星期几、第几节 → 点查询 → 空闲教室列表。

### 4️⃣ 看考试安排（即将上线）
菜单 → 「考试」→ 按时间排列的考试列表 → 每场考试显示倒计时天数。
