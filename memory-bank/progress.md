...
| **28** | 小组件课程时间有误 | 🟢 已实施·编译通过·视觉验收暂缓(先继续) | 小组件 | 小组件 `loadPeriodTimes` 用硬编码节次表；App 主程序 `fetchPeriods()` 拉真实节次仅存内存，Glance 独立进程取不到 → 永远用错表。**修复（#30 第一子项，2026-07-17(12)）**：`UserPreferences` 增 `periodTimesJson`(DataStore) 持久化真实节次；`ScheduleRepository.fetchPeriods` 成功拉取后写入；`ScheduleWidgetData.loadPeriodTimes` 改为读持久化真实节次（空则回退硬编码）。App 启动即写，Widget 跨进程读取，修 #28。**2026-07-17(13) 编译通过(agent自编译EXIT=0)+推送main；视觉验收(小组件时间正确性)暂缓待用户** |
| **29** | （新功能）查看已公布的下学期课表 | 🟢 已实施·agent自编译验证(EXIT=0)·待用户AS视觉验收 | 课表/新功能 | 管理员否决"选择器"/"自动切全局学期"；方案演进：Peek AlertDialog → 持久悬浮面板 → **三态复用同一 MD3 卡片**（2026-07-20，管理员要求"与询问时相同展示方式、保持一致性、复用素材"）。**最终落地**：`ScheduleUiState.panelMode: SemesterPanelMode`(NONE/DISCOVERY/REENTRY/BACK) 替代原 `showSemesterEndPanel` 布尔；`_currentYear/_currentTerm`(真实本学期) 与 `_viewYear/_viewTerm`(展示学期) 区分，`viewingNext`=二者不一致；"暂时忽略"改**内存态** `panelDismissedThisSession`（删 `UserPreferences.nextSemesterPeekDismissed` 键/DataStore），`onScheduleEntered()`(进页 `LaunchedEffect`) 重置该态并重算 → 满足"本次忽略、下次进页仍弹"。`computeAndApplyPanelMode()` 据「窗口态(`endWindowActive`:今天≥末周周日 且 <估算开学日) + 下学期是否已缓存Room(`hasCachedData`) + viewingNext」决定 mode：DISCOVERY(窗口内未缓存→"本学期已结束"+查询课表)、REENTRY(已缓存→"下学期课表已公布，是否查看？"+查看)、BACK(正在看下学期→"正在查看下学期课表"+返回本学期)。`queryNextSemesterSchedule()`(联网 refresh+切展示学期)、`viewNextSemester()`(仅切展示、不发网络)、`viewCurrentSemester()`(切回本学期)，均不碰全局 `currentSemester`；`switchDisplaySemester` 看下学期时置 `currentWeek=0` 防误高亮，`goToCurrentWeek` 对其回退到第1周。`ScheduleScreen.SemesterEndFloatingPanel` 按 `mode` 渲染同一卡片（标题/副标题/按钮文案由 `panelCopy` 映射，DISCOVERY/REENTRY 带"暂时忽略"、BACK 不带）。**编译通过(agent自编译EXIT=0)，待用户 AS 真机视觉验收** |
| **30** | 课表数据驱动化 / 小学期适配 | 🟡 已实施待验收 | 课表/结构性 | 4 子项已落地：① 小学期识别（`guessCurrentSemester` 加 `16` 分支，6月下旬–7月自动识别，N2154 校验兜底）；② 周数动态（`goToNextWeek` 改用 `lastWeek` 实际末周，去硬编码20）；③ 成绩年份动态（`getAllGrades` 改近3学年×[3,12] 自动生成，去硬编码2024/2025）；④ 校区去硬编码（`fetchPeriods` 读 `preferences.campusId` 替代写死"4"）。天数沿用 #31 开关未做数据驱动。**待用户在 Android Studio 编译 + 真机验收** |
...
| **33** | 课表卡片内容自适应（取消课名2行硬限） | 🟢 已验收通过·已推送main(dee2f2d) | 课表/UI/卡片 | 用户反馈：课名 `maxLines=2` 硬限致周末跨节课"拉成长条"、空间浪费。**实施（2026-07-17(9)）**：`CourseCard` 改用 `BoxWithConstraints` 读卡片真实内高 `maxHeight`；减教室(`innerH>=70dp`→2行否则1行)/老师固定占用+gap，得课名可用高→`nameMaxLines=max(1,(available/14dp).toInt())` 动态行数，长卡片自然填满、矮卡片仍紧凑；仅当总和快超才限行，单行仍溢出则 `overflow=Ellipsis`；极矮(`innerH<40dp`)课名10sp略缩。取消原硬限 `maxLines=2`。仅改 `CourseCard.kt`。**修订（2026-07-17(10)）**：初版用 `BoxWithConstraints` 致 `TextStyle` 在该作用域解析异常（报需 `spanStyle`/`paragraphStyle`）；改由父层 `WeekViewGrid` 传 `cardHeight: Dp`（= `rowHeight*spanCount-4.dp`）入参；并弃用 `TextStyle` 构造器、改用 `Text` 原生命名参数（`fontSize`/`fontWeight`/`lineHeight`/`color`，`lineHeight` 用 `sp`）。根因=误用 `LineBreak` 参数 + `lineHeight` 误传 `Dp`（应为 `TextUnit`），致 `TextStyle` 重载解析失败回退错误候选，产生一连串"无 fontWeight/fontSize/lineHeight"误导报错。agent 自行 `compileDebugKotlin` 通过（EXIT=0）。~~(验收要点同上，已于 2026-07-17(10) 用户验收通过)~~ |
...
| **2026-07-17 (10)** | **#33 验收通过 + push(`dee2f2d`)：课表卡片内容自适应落地。初版 `BoxWithConstraints` 致 `TextStyle` 在该作用域解析异常（误报需 spanStyle/paragraphStyle），改由 `WeekViewGrid` 传 `cardHeight:Dp` 入参 + 弃用 `TextStyle` 构造器改用 `Text` 原生命名参数（`lineHeight` 用 sp，修正误传 Dp + 误用 LineBreak 参数根因）。agent 自编译通过(EXIT=0)** | **用户验收 + push + 编译修复** |
| **2026-07-17 (11)** | **#30 勘察完成（待范围确认）：`ScheduleViewModel`——`guessCurrentSemester()` 只认秋季3/春季12、无小学期16分支；`detectAndApplySemester()` 仅自动识别当前学期；`dayLabels` 仅由"周末开关"决定5/7列（非数据驱动）；`periodLabels` 来自 `fetchPeriods` 仅内存缓存，widget `loadPeriodTimes` 硬编码副本(#28根因)；`goToNextWeek` 周数上限硬编码20。结合用户"否决常驻学期选择器"，#30 落地应以数据层支持枚举[3,12,16] + #29"下学期预览Peek"为入口，不动 selector UI** | **代码勘察** |
| **2026-07-17 (12)** | **#30 子项①（节次持久化 / 修 #28）实施完成·待验收**：用户选"节次持久化先行"。改动 3 文件——`UserPreferences`(增 `periodTimesJson` DataStore 键+Flow+setter)、`ScheduleRepository.fetchPeriods`(拉到真实节次后 `preferences.setPeriodTimesJson(periodsToJson(periods))` 持久化)、`ScheduleWidgetData.loadPeriodTimes`(改为读 `ep.userPreferences().periodTimesJson` 真实节次，空/解析失败回退硬编码 `hardcodedPeriods()`)。App 启动经 `loadInitialData→fetchPeriods` 写入，Widget 跨 Glance 进程读取，根除上述"永远用错表"。agent 自编译通过(EXIT=0)。**待用户 AS 课表页+小组件视觉验收** | **代码实施 + 编译** |
| **2026-07-17 (13)** | **#28/#30① 推送main**：节次持久化修复小组件时间错——`UserPreferences.periodTimesJson`(DataStore) 持久化真实节次、`ScheduleRepository.fetchPeriods` 写入、`ScheduleWidgetData.loadPeriodTimes` 跨进程读取（空则回退硬编码）。编译通过(EXIT=0) 已推送；**用户暂无法做小组件视觉验收，标记暂缓、先继续 #29**。补：#33 主行更正为「已验收通过·已推送main(dee2f2d)」** | **用户决策 + push + 记录修正** |
| **2026-07-17 (14)** | **#29 实施规划**：基于已定案 Peek 方案展开代码设计。`ScheduleViewModel` 抽 `_viewYear/_viewTerm` StateFlow（默认=detect 结果）替代闭包 year/term 驱动 `combine`，Peek 时切换、退出恢复，全程不改全局 `currentSemester`；新增 `nextSemester(y,t)` 在 `[3,12,16]` 取下一（16→次年3）。UI：`ScheduleScreen` 加 Peek 横幅（"正在查看下学期·返回本学期"）+ 学期末 AlertDialog 提示 + 复用 Snackbar 报"暂未公布"。待用户确认 2 个实现点（Peek 周次默认、触发时机）后实施 | **代码规划** |
| **2026-07-17 (15)** | **#29 实施完成·待验收**：改动 3 文件——`ScheduleViewModel`(抽 `_viewYear/_viewTerm` 流驱动 combine、`nextSemester(y,t)`([3,12,16]取下一,16→次年3)、`enterPeekNextSemester/exitPeek/dismissNextSemesterPrompt`、触发用实际末周 `autoWeek>=lastWeek`、节次转换复用 `toPeriodInfos`、`goToNextWeek` Peek 放宽至30)、`UserPreferences`(冷却键 `nextSemesterPeekDismissed`)、`ScheduleScreen`(Peek 横幅 Surface+点击 exitPeek、学期末 AlertDialog、未公布 Snackbar)。用户决策：Peek 默认第1周可翻周、触发=到达实际末周。agent 自编译验证(EXIT=0) | **代码实施 + 编译** |
| **2026-07-17 (17)** | **#29 方案二度变更：Peek→持久悬浮面板 + 编译修复**：管理员推翻 Peek（AlertDialog+横幅），要求「从第20周周日结束起**持久浮于课表网格下半区、不压暗背景、遵循MD3**的浮动卡片，持续到下学期开学日才收起」。`ScheduleViewModel` 废弃 Peek 字段、改面板字段（`showSemesterEndPanel`/`nextTermLabel`/`queryingNextSemester`/`nextSemesterCoursesLoaded`/`nextSemesterUnavailable`/`lastWeek`）；`loadInitialData` ⑥ 据「今天 ≥ 末周周日(N2154 末周 end date) 且 今天 < 估算开学日(秋→次年2/20,春/小学期→次年8/25) 且 本学期未忽略」算可见性；`goToNextWeek` 恢复上限20；`dismissSemesterEndPanel`(按 `year-term` 持久冷却)/`queryNextSemesterSchedule`(切 `_viewYear/_viewTerm` 展示学期+拉节次+refresh，成功置 `nextSemesterCoursesLoaded`、失败置 `nextSemesterUnavailable`)。`ScheduleScreen` 删 Peek 全部 UI，加 `SemesterEndFloatingPanel`(MD3 Surface 卡片：主标题"本学期已结束"、副标题含下学期标签、左"暂时忽略"文字按钮+右"查询课表"胶囊按钮带 loading)。修复 `ScheduleUiState` 第47行缺逗号语法错 + `nextTermLabel` 副标题融入。agent 自编译 `compileDebugKotlin` EXIT=0 BUILD SUCCESSFUL。**待用户 AS 真机视觉验收** | **方案变更 + 代码实施 + 编译修复** |
| **2026-07-20 (18)** | **#29 三态浮层改造 + 会话态忽略 + 再入口**：管理员确认"暂时忽略"应为**仅本次进入忽略、下次进页仍弹**（此前误按 Peek 方案做成 DataStore 持久冷却）；并要求再入口"与询问时相同展示方式、复用素材"。改造：`ScheduleUiState.showSemesterEndPanel`→`panelMode: SemesterPanelMode`(NONE/DISCOVERY/REENTRY/BACK)；`ScheduleViewModel` 新增 `_currentYear/_currentTerm`(真实本学期) 与 `panelDismissedThisSession`(内存态) + `endWindowActive` 窗口标志；`onScheduleEntered()`(进页 `LaunchedEffect`) 重置忽略态并重算；`computeAndApplyPanelMode()` 据「窗口态+下学期已缓存Room+viewingNext」定 mode：DISCOVERY(窗口内未缓存→"本学期已结束"+查询课表)、REENTRY(已缓存→"下学期课表已公布，是否查看？"+查看)、BACK(看下学期→"正在查看下学期课表"+返回本学期)；`queryNextSemesterSchedule`(联网)/`viewNextSemester`(仅切展示不发网络)/`viewCurrentSemester`(切回本学期) 均不碰全局；`switchDisplaySemester` 看下学期置 `currentWeek=0` 防误高亮，`goToCurrentWeek` 回退第1周；删 `UserPreferences.nextSemesterPeekDismissed` 键(Flow/setter/DataStore)。`ScheduleScreen.SemesterEndFloatingPanel` 按 `mode` 复用同一 MD3 卡片(`panelCopy` 映射文案，DISCOVERY/REENTRY 带"暂时忽略"、BACK 不带)，加进页 `LaunchedEffect` + 未公布 Snackbar 消费。AS `assembleRelease` 报 3 错(未定义方法+旧字段)→ agent 补齐方法并自编译 `compileDebugKotlin` EXIT=0 BUILD SUCCESSFUL。**待用户 AS 真机视觉验收** | **方案确认 + 代码实施 + 编译修复** |
| **2026-07-20 (19)** | **#30 小学期适配实施·待验收**：4 子项落地——① 小学期自动识别 `guessCurrentSemester()` 加 term `16` 分支(6/20–7月→上年小学期，8–12月→秋3，其余→春12，经 `detectAndApplySemester` N2154 校验兜底，小学期无数据回退春)；② 周数动态化 `goToNextWeek` 改用 `lastWeek`(由 N2154 周次映射算实际末周) 替代硬编码20；③ 成绩年份动态化 `getAllGrades` 改近3学年(今年/去年/前年)×[3,12] 生成，剔除写死2024/2025；④ 校区去硬编码 `fetchPeriods` 读 `preferences.campusId` 替代写死"4"。天数沿用 #31 周末开关未做数据驱动。agent 未自编译(用户在 AS 编译+真机验收) | **代码实施** |
| **2026-07-21** | **#25 首页 cache-first 实施·待验收**：首页 `loadHomeData` 改为纯读本地缓存、启动零联网（移除 fetchPeriods/getAllGrades 启动拉取 + isLoading 等待）；下场考试改读 Room 缓存 `getCachedExams`；今日课读 Room + 本地节次时间表(`periodTimesJson`)。新增 `UserPreferences` 两键缓存"本学期加权均分"；`GradeViewModel` 刷新后用 `GpaCalculator` 算最新学期均分写入缓存（**顺带修正首页原手写加权公式未剔除无分课程、导致比成绩页偏低的 bug**）。`ScheduleRepository` 抽出文件级 `periodsFromJson`/`fallbackPeriods` 供首页与小组件共用。`HomeViewModel` 加 `observeCache` 让卡片返回首页即时刷新。未自编译(用户在 AS 验收) | **代码实施** |
...
| 25 | 首页预览需等 1-2s 才出现 | 🟡 已实施 | 已实施 cache-first：首页 `loadHomeData` 纯读本地缓存、启动零联网；刷新移至各页面进入时触发；成绩加权均分改由成绩页用 `GpaCalculator` 计算并写入 DataStore 缓存（顺带修正首页原手写公式与成绩页不一致的 bug）。**待用户 AS 编译 + 真机验收** | `HomeViewModel.kt` / `UserPreferences.kt` / `GradeViewModel.kt` / `ScheduleRepository.kt` |
