...
| **30** | 课表数据驱动化 / 小学期适配 | 🟡 进行中·子项①节次持久化已随#28推送main | 课表/结构性 | HAR 查明：学期下拉=HTML `<select>`、**无独立 JSON 列表接口**；且 **term 码 `16`=第三学期/小学期已内置**。据此改为：学期枚举 `[3,12,16]`(含小学期) + 天数数据驱动(max xqj) + 节次持久化共用 + 周数动态。详见「硬编码清单 & 小学期适配评估(修订)」|
